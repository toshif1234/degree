...
//if($r_e['elective'] == 0){
//$q="select usn,fname,mname,lname from students where branch=\"" . $branch . "\" and semester=\"" . $sem . "\" and section=\"" . $sec . "\"";
$q = "select s.usn,s.fname,s.mname,s.lname from students s, subjects_new n where s.branch=\"" . $branch . "\" and s.semester=\"" . $sem . "\" and s.section=\"" . $sec . "\" and n.sem=s.semester and n.sub_name=\"" . $sub . "\" and n.branch=s.branch";
//}
// else{
...
foreach ($r as $result) {
    ...
    $q5 = "SELECT n.usn,n.subj from nodue n , students s,subjects_new k where n.usn='" . $result["usn"] .  "'and s.section='" . $sec . "' and subj='" . $sub . "' and  k.sem=s.semester and k.sub_name=\"" . $sub . "\" and k.branch=s.branch and n.sem=k.sem and n.sec=s.section";
    $result6 = $link->query($q5);
    // echo    $q5;

    $cont = "MENTOR";

    $q9 = "SELECT usn,subj from nodue where usn='" . $result["usn"] .  "'and subj='" . $cont . "'";
    $result9 = $link->query($q9);
    ...
    if (mysqli_num_rows($result6)) {
        ...
        $q3 = "INSERT INTO `nodue`( `usn` ,`name`,`subj`,`sem`,`sec`,`due`) VALUES ('" . $result["usn"] .  "','" . $result["fname"] .  "','" . $sub . "','" . $sem . "','" . $sec . "','" . $content . "');";
        // $q4="INSERT INTO `nodue`( `usn` ,`name`,`subj`,`due`) VALUES ('" . $result["usn"] .  "','" . $result["fname"] .  "','" . $cont . "','" . $content . "');";
        /// echo $q3;

        try {

            $result2 = $link->query($q3);
            // $result9=$link->query($q4);
    ...
    <div class="row">
        ...
        <table class="table table-hover mt-4">
                        ...
                        <form action=" " method="post">
                            ...
                            <td>
                                <select id="id" name="id">
                                    ...
                                    <option selected value="<?php echo $r['due'] ?>"><?php echo $r["due"] ?></option>
                            ...
                            <td><button class="btn btn-primary btn-sm" type="submit" formaction="arrange.php">SAVE</button></td>
            ...
            <!-- <p align="right">
                ...
                <button class="btn btn-primary btn-sm"  style="height:40px; width:150px" type="submit" formaction="save.php">SAVE ALL</button>
                     </p> -->
            </tbody>
        </table>
