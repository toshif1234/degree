...
$q='select * from students where usn="'.$_SESSION['username'].'"';
        ...
        $_SESSION['feedback_number'] = $_GET['number'];
        $feedback_name = 'Course wise ' . $_GET['number'];

        $q_subjects='select sub from feedback_response where usn="'.$_SESSION['username'].'" and feedback_name="' . $feedback_name . '"';
...
<form action="update_course_wise.php" method="post">
    <div class="row m-5">
    ...
    <input type="text" name="feedback_name" id="" value="<?php echo $feedback_name ?>" hidden>
