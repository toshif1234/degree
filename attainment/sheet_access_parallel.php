...
$q_sub_fac = 'select * from faculty_mapping where sub_name = "' . $sub . '"';
$teach = mysqli_fetch_assoc($link->query($q_sub_fac))['faculty_name'];
$q_branch = 'select faculty_dept from faculty_details where faculty_name="' . $teach . '";';
$result2 = $link->query($q_branch);
$dept = "Computer Science and Engineering-parallel";
...
$q_target = 'select * from targets where dept = "Computer Science and Engineering" and batch = "' . $_POST['batch'] . '"';
...
$q_assignment = 'select * from add_assignment where semester = "' . $sem . '" and branch = "' . $dept . '" and section = "C" and sub_name = "' . $sub_name . '" order by assignment_no';
...
?>
<!-- <a href="attainment_<?php echo $dept ?>.xlsx" download>111</a> -->