...
<form action="lab_parallel_sheet_access.php" method="POST">
    <div class="form-row">
        ...
        <div class="col-md-6 mb-3">
            ...
            <select class="form-control" name="sub" id="sub" aria-label="Default select example">
                ...
                if (isset($_SESSION['is_archive']) && $_SESSION['is_archive'] == 1) {
                    $qt = 'selet * from subjects where branch = "' . $dept . '"';
                } else {
                    $qt = 'selet * from subjects_new where branch = "' . $dept . '"';
                }
                if (isset($_SESSION['dept_admin']) && $_SESSION['dept_admin'] == 1) {
                    if (isset($_SESSION['is_archive']) && $_SESSION['is_archive'] == 1) {
                        $qt = 'select * from subjects where branch = "' . $dept . '"';
                    } else {
                        $qt = 'select * from subjects_new where branch = "' . $dept . '"';
                    }
                } else {
                    if (isset($_SESSION['is_archive']) && $_SESSION['is_archive'] == 1) {
...
<a hidden href="labattainment_Computer Science and Engineering-parallel.xlsx" download id="down"></a>
<?php
